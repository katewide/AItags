...
const OPEN_TASK_ANALYSIS_TIMEOUT_MS = Number(process.env.OPEN_TASK_ANALYSIS_TIMEOUT_MS || 300 * 1000);
...
const TASK_TAGGING_PROMPT_PATH = path.join(__dirname, 'prompts', 'task-tagging.md');
const TASK_TAXONOMY_PATH = path.join(__dirname, 'config', 'task-taxonomy.json');
const TASK_TAGGING_PROMPT = fs.readFileSync(TASK_TAGGING_PROMPT_PATH, 'utf8');
const TASK_TAXONOMY = JSON.parse(fs.readFileSync(TASK_TAXONOMY_PATH, 'utf8'));
...
const recentAiTagUpdates = new Map();
...
Название компании:
- при дословном OCR сохраняй написание с изображения;
- если из контекста ясно, что речь о компании ЭЛРОС, считай вариантами её названия «Элрос», «элрос», а также возможные ошибки распознавания «ЭРОС», «ЭПРОС» и похожие варианты;
- в перечислении фактов используй каноническое написание «ЭЛРОС»;
- не нормализуй название, если из контекста не следует, что это именно компания ЭЛРОС.
  ...
  if (previousRows.length === 0) {
    if (currentEntries.length > 0) {
      ...
      for (const currentEntry of currentEntries) {
        const userId = currentEntry.userId || null;
        ...
        if (change.diffMinutes !== 0) {
          changes.push({
            ...change,
            detail_source: 'current_time_entries_without_baseline',
          });
        }
      }
...
async function runTaskTimeEntryBaseline() {
...
function isTaskSummaryFieldChange(change) {
  return normalizeHistoryField(change?.field) === normalizeHistoryField(TASK_SUMMARY_FIELD_CODE);
}
...
function normalizeTaskTag(value) {
  ...
  return String(value.name || value.NAME || value.title || value.TITLE || '').trim();
}

function getTaskTags(task) {
  const value = task?.tags ?? task?.TAGS ?? task?.tagNames ?? task?.TAG_NAMES ?? [];
  ...
  return [...new Set(rawTags.map(normalizeTaskTag).filter(Boolean))];
}
...
function extractTaskTagClassification(aiComment) {
  ...
  try {
    ...
    const type = TASK_TAXONOMY.types.includes(parsed?.type) ? parsed.type : null;
...
function stripTaskTagBlock(aiComment) {
  return String(aiComment || '')
    .replace(/\s*\[AI_TAGS\][\s\S]*?\[\/AI_TAGS\]\s*/gi, '')
    .trim();
}
...
function markRecentAiTagUpdate(taskId) {
  recentAiTagUpdates.set(String(taskId), Date.now());
}

function isRecentAiTagUpdate(taskId) {
  const normalizedTaskId = String(taskId);
  const updatedAt = recentAiTagUpdates.get(normalizedTaskId);
  if (!updatedAt) return false;
  if (Date.now() - updatedAt > 10 * 60 * 1000) {
    recentAiTagUpdates.delete(normalizedTaskId);
    return false;
  }
  return true;
}
...
async function updateTaskTags(taskId, classification, currentTask = null) {
  ...
  const existingTags = getTaskTags(currentTask);
  const mergedTags = mergeTaskTags(existingTags, classification);
  if (taskTagListsEqual(existingTags, mergedTags)) {
    return { updated: false, skipped: true, reason: 'same_value', tags: mergedTags, error: null };
  }

  markRecentAiTagUpdate(taskId);
  try {
    await coworkRequest('PATCH', `/tasks/${taskId}`, {
...
function buildTaskTaggingInstructions() {
  if (!TASK_TAGGING_ENABLED) return '';
  return TASK_TAGGING_PROMPT.replace(
    '{{TASK_TAXONOMY}}',
    JSON.stringify(TASK_TAXONOMY, null, 2)
  );
}
...
НАЗВАНИЕ КОМПАНИИ:
- если из JSON ясно, что речь о компании ЭЛРОС, считай вариантами её названия «Элрос», «элрос», а также вероятные OCR-ошибки «ЭРОС», «ЭПРОС» и похожие варианты;
- в итоговом тексте всегда используй каноническое написание «ЭЛРОС»;
- не заменяй название, если из контекста не следует, что это именно компания ЭЛРОС.
Запрещено:
...
- Если номера релизов совпадают или указано, что база обновлена на целевой релиз:
...
Если сведения противоречат друг другу:
- комментарии имеют приоритет над описанием задачи;
- более поздние комментарии и события имеют приоритет над более ранними;
- сведения из текущей задачи имеют приоритет над сведениями из родительской задачи, если речь идет о текущем состоянии работ.

ПРИНЦИП ДОКАЗАТЕЛЬНОСТИ:
Любое утверждение в результате должно быть прямо подтверждено JSON.
Если факт нельзя подтвердить исходными данными, он считается отсутствующим.
Если технический факт подтвержден исходными данными и существенен для понимания причины ожидания, выполненных работ или следующего шага, не удаляй его ради краткости и не заменяй общими словами.

НАЗВАНИЕ КОМПАНИИ:
- если из JSON ясно, что речь о компании ЭЛРОС, считай вариантами её названия «Элрос», «элрос», а также вероятные OCR-ошибки «ЭРОС», «ЭПРОС» и похожие варианты;
- в summary всегда используй каноническое написание «ЭЛРОС»;
- не заменяй название, если из контекста не следует, что это именно компания ЭЛРОС.

Запрещено:
- делать предположения;
- использовать типичные знания о 1С вместо фактов из JSON;
- достраивать цепочку действий;
- придумывать договоренности, сроки, результаты проверки или выполненные работы.
...
ДОПУСТИМЫЕ ЗНАЧЕНИЯ И ПРАВИЛА ИХ ВЫБОРА:
...
next_check_date: календарная дата следующей проверки в формате YYYY-MM-DD.
- Всегда ставь next_check_date = currentDate + ${OPEN_TASK_DEFAULT_RECHECK_DAYS} дня.
- Не изменяй next_check_date из-за сроков, звонков, отпусков, дедлайнов или комментариев вида “ждем до ...”.
...
summary:
- summary должен быть одним или двумя деловыми предложениями о текущем статусе задачи.
- Полнота и доказательность информации важнее краткости.
- Укажи только те сведения, которые прямо подтверждены JSON: что именно ожидается, от кого ожидается следующий шаг, какие существенные действия уже выполнены, какая существенная техническая причина или результат подтверждены.
- Если информации мало, не дополняй ее предположениями. Напиши только то, что подтверждено JSON, и явно укажи, что дополнительные сведения отсутствуют.
- Например, если из JSON подтверждено только “написали клиенту”, верни: "По задаче клиенту направлено сообщение. Дополнительные сведения отсутствуют."`;
}
...
async function processClosedTask(taskId, options = {}) {
  ...
  if (dryRun) {
    if (TASK_TAGGING_ENABLED && tagClassification.found && (tagClassification.type || tagClassification.products.length > 0)) {
      ...
      taskTagsResult = {
        updated: false,
        skipped: !taskTagsWouldBeUpdated,
        reason: taskTagsWouldBeUpdated ? 'dry_run' : 'same_value',
        tags: mergedTags,
        error: null,
      };
    }
  ...
  return {
    ...
    task_tags_would_be_updated: taskTagsWouldBeUpdated,
    task_tags_skipped: taskTagsResult.skipped,
    task_tags_skip_reason: taskTagsResult.reason,
    task_tags_error: taskTagsResult.error,
    raw_ai_comment: rawAiComment,
...
async function handleWebhook(body) {
  ...
  if (tagChange && isRecentAiTagUpdate(taskId)) {
    saveDebug('lastTaskCloseDecision', {
      task_id: taskId,
      webhook_ts: data.ts || null,
      primary_field: primaryChange?.field || null,
      ...
      should_process_closed_task: false,
      close_trigger: null,
      ignored_reason: 'ai_generated_tag_update',
    });
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if ((req.method === 'GET' || req.method === 'POST') && pathname === '/time-entry-baseline') {
      if (req.method === 'POST') await readBody(req);
      const result = await runTaskTimeEntryBaseline();
      sendJson(res, 200, result);
      return;
    }
...
server.listen(PORT, () => {
  ...
  // Ручной запуск через /open-tasks-check сохранён для отладки.
  /*
